    ...
    users.forEach((user, index) => {
      ...
      if (user.fcmTokens && user.fcmTokens.length > 0) {
        user.fcmTokens.forEach((token, tokenIndex) => {
          ...
          console.log(
            `   ${tokenIndex + 1}. ${status} - ${token.platform} - ${token.token.substring(0, 20)}...`,
          )
        })
      }
    })
      ...
      if (activeTokens.length > 0) {
        ...
        const testResult = await sendFCMNotificationToUsers(
          [testUser._id.toString()],
          {
            ...
            body: 'This is a test notification from Docora backend',
          },
          {
        ...
        if (testResult.responses) {
          testResult.responses.forEach((response, index) => {
            if (!response.success) {
              console.log(
                `   Failed Token ${index + 1}: ${response.error?.message || 'Unknown error'}`,
              )
            }
          })
        }
