...
export const register = catchAsync(async (req, res) => {
  ...
  try {
    ...
    console.log(
      '🔵 [REGISTER] Referral system enabled:',
      settings.referralSystemEnabled,
    )
    ...
    console.log(
      ...
      newUser ? { id: newUser._id, email: newUser.email } : 'NULL',
    )
    if (!newUser) {
    ...
    console.log(
      '🔵 [REGISTER] Checking if notifications needed for role:',
      roleNormalized,
    )
    if (roleNormalized === 'doctor') {
      console.log(
        '🔵 [REGISTER] Sending notifications to patients about new doctor',
      )
...
export const forgetPassword = catchAsync(async (req, res) => {
  ...
  try {
    ...
    await sendEmail(user.email, 'Password Reset OTP - Docora', emailHtml)
  } catch (emailError) {
    throw new AppError(
